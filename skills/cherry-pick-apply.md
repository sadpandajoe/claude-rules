...
3. For excluded files that **don't exist on the target branch**, expect modify/delete conflicts — these must be resolved with `git rm` during conflict resolution, not reverted after. See the Modify/Delete Conflicts section below.
...
1. **Resolve in place** — `git rm` for modify/delete, edit markers for content conflicts, then `git cherry-pick --continue`.
